---
title: Implementation Plan - Red-Fastapi (Python Backend for Node-RED Frontend)
...
# Implementation Plan: Red-Fastapi (Python Backend for Node-RED Frontend)

**Project Name:** `red-fastapi`  
...
```
 ...
 ┌───────────────────────────────────────┴────────────────────────────────────────┐
 │                   Red-Fastapi Engine (Runtime Layer)                           │
...
- [x] Built node registry subsystem (`red_fastapi.runtime.registry`) mirroring `@node-red/registry`.
...
- [x] Implemented storage layer (`red_fastapi.runtime.storage`) writing to `storage/flows.json` with MD5 revision (`rev`) tracking.
- [x] Implemented flow parser and diffing utility (`red_fastapi.runtime.flows_util`).
- [x] Implemented runtime flow management (`red_fastapi.runtime.flows`).
- [x] Implemented flows API router (`red_fastapi.api.flows`): `GET/POST /flows`, `GET/POST /flows/state`, and workspace CRUD.
- [x] Validated with automated tests (`tests/test_phase3.py`).

### Phase 4: Real-time Event Bus (WebSocket `/comms`) `[COMPLETED]`
- [x] Connection lifecycle & client connection registry (`red_fastapi.runtime.comms`).
